...
def check_pattern():
    ...
    for tile in sense_nearby_map_infos(radius_squared=8):
        ...
        if tile.get_paint().is_enemy():
            return False
        if tile.get_paint() == PaintType.EMPTY or tile.get_paint().is_secondary() != get_is_secondary(cur_loc, tile_loc, UnitType.LEVEL_ONE_MONEY_TOWER):
            incorrect_paint += 1
    return incorrect_paint <= 25
...
def run_tower():
    ...
    if (cur_type == UnitType.LEVEL_ONE_MONEY_TOWER or cur_type == UnitType.LEVEL_TWO_MONEY_TOWER) and check_pattern() == True and has_nearby_robots() and sense_nearby_robots(team=get_team().opponent()) and get_money() > 2000:
